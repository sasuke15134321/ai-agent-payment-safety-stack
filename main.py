...
class PaymentCounterpartyInput(BaseModel):
    name: Optional[str] = None
    domain: Optional[str] = None
    wallet: Optional[str] = None


class PaymentContextStateInput(BaseModel):
    status: str = "unknown"
...
class PaymentPolicyInput(BaseModel):
    max_amount_per_payment: float = 0.10
    allowed_currencies: List[str] = ["USDC"]
    require_human_approval_above: float = 0.10
    block_unknown_counterparty: bool = False
    require_payment_evidence: bool = True


class PaymentReviewCheckRequest(BaseModel):
    agent_id: str
    user_id: Optional[str] = None
    amount: float
    currency: str
    counterparty: Optional[PaymentCounterpartyInput] = None
    payment_purpose: Optional[str] = None
...
async def payment_review_check(req: PaymentReviewCheckRequest):
    ...
    if req.source_text:
        ...
        for pattern in _INJECTION_PATTERNS:
            if pattern in lower_text:
                ...
                checks.append({"name": "injection_check", "result": "deny", "reason": f"Dangerous pattern detected: '{pattern}'"})
                deny_reasons.append(f"injection pattern: {pattern}")
                break
    ...
    elif context_state.status in ("unknown", "stale", "historical"):
        checks.append({"name": "context_state_check", "result": "review_required", "reason": f"Context state is {context_state.status}"})
        review_reasons.append(f"context state is {context_state.status}")
    else:
    ...
    if not has_counterparty and not deny_reasons:
        ...
        review_reasons.append("counterparty is unknown")
    else:
    ...
    if deny_reasons:
        ...
        recommended_action = "Block this payment. Do not proceed."
        human_review_required = False
    elif review_reasons:
        decision = "review_required"
        risk_level = "medium"
        reason = "; ".join(review_reasons)
        recommended_action = "Route to human review before proceeding."
    ...
    if req.amount > policy.max_amount_per_payment:
        ...
        budget_status = "requires_human_approval"
    else:
        budget_status = "within_policy"

    counterparty_status = "provided" if has_counterparty else "unknown"
    injection_risk = "detected" if injection_detected else "none"

    if req.requested_tool in _DANGEROUS_TOOLS:
        tool_permission_status = "blocked"
    elif req.requested_tool:
        tool_permission_status = "allowed"
    else:
        tool_permission_status = "not_requested"
    ...
    return {
        "review_id": review_id,
        "review_type": "agent_payment_review",
        "status": "created",
        "experimental": True,
        "stateless": True,
        "agent_id": req.agent_id,
...
async def agent_json():
    ...
    return {
        ...
        "endpoints": [
            ...
            {
                "name": "Agent Action Atom Builder",
                "method": "POST",
                "path": "/api/action-atom/build",
                "pricing": "free",
                "x402_required": False,
                "stateless": True,
                "experimental": True,
                "general_purpose": True,
                "purpose": "Converts one AI-agent action into a structured external control record with intent, target, context state, checks, decision, evidence, and audit-readiness.",
                "positioning": "Currently hosted in ai-agent-payment-safety-stack as the first payment-related use case, but designed as a general-purpose external control record unit.",
                "can_be_combined_into": [
                    "Agent Payment Action Record",
                    "Payment Control Evidence Packet",
                ],
            ...
            {
                "name": "Agent Payment Review API",
                "method": "POST",
                "path": "/api/payment-review/check",
                "pricing": "free",
                "x402_required": False,
                ...
                "use_when": [
                    "an AI agent needs a payment review before using a paid API",
                    "a system needs to decide whether to allow, deny, or require human review for an agent payment",
                    "a developer needs an evidence_id for a payment decision",
                ],
                "returns": [
                    "decision", "risk_level", "reason", "recommended_action",
                    "budget_status", "counterparty_status", "injection_risk",
                    "tool_permission_status", "memory_context_status", "evidence_id",
                ],
                "can_feed_into": [
                    "Agent Payment Action Record",
                    "Payment Control Evidence Packet",
        ...
        "external_control_materials": {
            "name": "External Control Materials for AI-Agent Trust, Tool Use, and Payments",
            "version": "0.4",
            "map_endpoint": "/.well-known/external-control-materials.json",
            "purpose": "AI-readable map showing how memory provenance, search result trust, tool permission, tool approval, observability boundaries, command execution gates, spending policy, payment review, budget checks, payment records, and evidence packets fit together.",
            "recommended_flow": [
            ...
            "pricing_summary": {
                "free_materials": "builders, policies, records, packets, maps, command gates, tool approval, payment review, search trust check",
...
async def external_control_materials_json():
    return {
        ...
        "positioning": "External control materials for AI-agent memory, search result trust, tool permission, tool approval, spending, token budget, payment decision, and evidence workflows.",
        ...
        "recommended_flow": [
            {
                "step": 1,
                "name": "Memory Provenance Context Record",
                "service": "agent-memory-api",
                "endpoint": "POST /api/memory-provenance-record/build",
                "pricing": "free",
                "x402_required": False,
                "purpose": "Check whether a memory or context item can be used for tool, spending, token, or payment decisions.",
                "material_type": "memory_provenance",
                "output": ["raw_sources", "facts", "profile_or_context_summary", "state", "use_rule", "evidence", "last_checked", "risk_flags"],
            },
            {
                "step": 2,
                "name": "Search Result Trust Check",
                "service": "agent-memory-api",
                "endpoint": "POST /api/search-result-trust/check",
                "live_url": "https://agent-memory-api-bix5.onrender.com/api/search-result-trust/check",
                "pricing": "free",
                "x402_required": False,
                ...
                "material_type": "search_result_trust_decision",
                "output": ["trust_check_id", "decision", "risk_level", "reason", "recommended_action", "freshness_status", "provenance_status", "contradiction_status", "source_trust_status", "intended_use_status", "evidence_id", "agent_action_atom"],
            },
            {
                "step": 3,
                ...
                "pricing": "free",
                "x402_required": False,
                "purpose": "Define which tools, APIs, memory operations, network access, and sensitive actions an AI agent may use.",
                "material_type": "tool_permission_policy",
                "output": ["allowed_tools", "blocked_tools", "approval_rules", "risk_boundaries", "context_state", "agent_action_atom"],
            },
            {
                "step": 4,
            ...
            },
            {
                ...
                "pricing": "free",
                "x402_required": False,
                "purpose": "Treat Sentry issues, logs, tickets, CI outputs, alerts, and other observability data as untrusted operational data before they enter AI-agent context.",
                "material_type": "observability_data_boundary",
            ...
            },
            {
                ...
                "pricing": "free",
                "x402_required": False,
                "purpose": "Create a command execution gate record before running shell commands derived from untrusted observability data such as Sentry issues, logs, tickets, alerts, or CI outputs.",
                "material_type": "command_execution_gate",
            ...
            },
            {
                ...
                "pricing": "free",
                "x402_required": False,
                "purpose": "Define spending limits, approval rules, token budget, memory scope policy, reasoning cost boundary, and human review triggers.",
                "material_type": "spending_policy",
            ...
            },
            {
                ...
                "pricing": "x402",
                "x402_required": True,
                "purpose": "Check whether a paid API call is within the AI agent's budget and spending policy.",
                "material_type": "paid_budget_check",
            ...
            },
            {
                ...
                "pricing": "free",
                "x402_required": False,
                "purpose": "Create a payment decision record from payment intent, pre-payment checks, post-payment evidence, context state, decision, and Agent Action Atom.",
                "material_type": "payment_action_record",
            ...
            },
            {
                ...
                "pricing": "free",
                "x402_required": False,
                "purpose": "Package payment intent, checks, evidence, Agent Action Atom, and Agent Payment Action Record into one reviewable evidence packet.",
                "material_type": "evidence_packet",
            ...
            },
            {
                ...
                "pricing": "x402",
                "x402_required": True,
                "purpose": "Check payment evidence after a paid API usage or payment flow.",
                "material_type": "paid_payment_evidence_check",
                "output": ["evidence_status", "payment_status", "risk_flags", "recommendation"],
            },
        ],
        "free_materials": [
        ...
        "pricing_model": {
            "free": "Builders, maps, records, policies, and evidence packaging are free because they create structure and external control material.",
            "paid": "Actual checks, validation, risk detection, payment evidence verification, budget decisions, and memory operations may be paid x402 endpoints.",
        },
        "use_when": [
            "an AI agent needs external control material before using a paid API",
            "an AI agent needs to know which memory can be used before a decision",
            "an AI agent needs tool permission boundaries before calling external APIs",
            "an AI agent needs token budget and spending limits before reasoning or selecting tools",
            "an AI agent needs to create a payment decision record",
            "an AI agent needs to package evidence for review",
            ...
            "an AI agent needs to verify whether a retrieved result, observation, or record can be trusted before using it for tool execution, payment, or operational decisions",
        ],
        "non_goals": [
            "not an AI OS",
        ...
        "fractal_structure": {
            ...
            "trust_gates": ["search_result_trust_check", "tool_approval_check", "payment_review_decision"],
        },
        "okf_bundle": {
            "index": "/okf/index.md",
            "version": "0.1",
